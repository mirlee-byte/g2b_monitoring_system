...
# (고급) AI 채점 기준 문구는 이 파일 아래 prompt 안에 있습니다 — 수정 시 개발자와 상의하세요.
"""
...
# 작성 원칙 (반드시 준수)
- 바쁜 실무자가 5초 안에 읽도록 간결하게. 한 항목은 한 가지만 말한다.
- 특허번호(PAT_x)·인증명·업종코드를 줄줄이 나열하지 말 것. 꼭 필요하면 대표 1개만.
- key_points는 2개, 각 항목 한 문장(공백 포함 45자 이내)으로 핵심만.
- concerns는 1~2개, 각 한 문장(45자 이내).
- recommendation은 1~2문장으로 압축. 미사여구·중복 표현 금지.
- 단, reason(검토 의견)은 적격 판단의 근거를 충실히 담아 2~3문장으로 작성한다.

다음 JSON 형식으로만 응답하세요. 다른 텍스트 없이 순수 JSON만 출력:
{{
    "score": <0~10 정수>,
    "eligible": <true/false>,
    "reason": "<점수를 준 핵심 근거를 충실히 담은 2~3문장>",
    "key_points": ["<강점1: 한 문장 45자 이내>", "<강점2: 한 문장 45자 이내>"],
    "concerns": ["<우려1: 한 문장 45자 이내>"],
    "recommendation": "<참여 여부와 핵심 전략 1~2문장>"
}}"""
